...
HMODULE getmanmodulehandleppe(const wchar_t* dllName) {
    // Get PEB from TEB using GS segment
    UINT_PTR teb = __readgsqword(0x30);                    // TEB
    UINT_PTR peb = *(UINT_PTR*)(teb + 0x60);               // TEB->ProcessEnvironmentBlock
    UINT_PTR ldr = *(UINT_PTR*)(peb + 0x18);               // PEB->Ldr
    UINT_PTR moduleList = ldr + 0x10;                      // &Ldr->InLoadOrderModuleList

    UINT_PTR currentEntry = *(UINT_PTR*)(moduleList);      // ListEntry->Flink
    UINT_PTR listHead = moduleList;

    while (currentEntry != listHead) {
        UINT_PTR baseDllNamePtr = currentEntry + 0x58;     // UNICODE_STRING BaseDllName
        UNICODE_STRING* baseDllName = (UNICODE_STRING*)baseDllNamePtr;
        WCHAR* buffer = baseDllName->Buffer;

        if (buffer) {
            if (_wcsicmp(buffer, dllName) == 0) {
                // Use OriginalBase instead of DllBase to bypass PAGE_GUARD
                UINT_PTR originalBase = *(UINT_PTR*)(currentEntry + 0xF8);  // OriginalBase offset
                wprintf(L"Found module (original base): %s at address: %p\n", buffer, (void*)originalBase);
                return (HMODULE)originalBase;
            }
        }

        currentEntry = *(UINT_PTR*)currentEntry;  // Move to next Flink
    }
